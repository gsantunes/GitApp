...
class ViewController: UIViewController {
    ...
    func getAllUserRepos(user:NSString){
...
//        var allRepos = NSURL(string: "https://api.github.com/users/\(username)/repos") <-- puxando da web
        ...
        if let repos: AnyObject = json{
            for items in repos as! NSArray{
                ...
                checkARepo(nome2)  //chamando a função para checar o dono do repo
...
func checkARepo(repo:NSString)  {
    

    
      var repos = NSURL(string: "file:///Users/pardim/Desktop/Contador.html") //<--usando arquivo local
//        var repos = NSURL(string: "https://api.github.com/repos/wellpardim/\(repo)")
...
//        println(dic3["login"])
    ...
    func checkPullNumber(repoName:NSString) {
        ...
        // se ocorrer um crash na linha acima, pode ser que as requisicões acabaram
            ...
            if let repos: AnyObject = json{
                for repo in repos as! NSArray{
                    ...
                    println("-----")
                    println(dic2)
                    if("bruom" == dic2){  //acessando todos os pull requests do repo pra procurar para encontra o pull request do usuario
...
//        var repos = NSURL(string: " https://api.github.com/repos/mackmobile/\(number)/issues/\(number)")
